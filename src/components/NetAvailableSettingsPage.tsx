...
import { getSettings, getNetAvailableConfig, isConfigured, updateNetAvailableConfig } from '../services/settings';
...
export default function NetAvailableSettingsPage({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { totalAssets: _, totalLiabilities: __, netAvailable: ___, ...raw } =
        dash.financial;
      setFinancial(raw);
    } catch (err) {
  ...
  return (
    <div className="net-available-settings-page">
      <div className="card dashboard-hero-card net-available-preview-card">
        ...
        <MoneyDisplay
          amount={preview?.netAvailable ?? 0}
          size="hero"
          tone="hero"
        />
        <p className="dashboard-hero-hint">
          بر اساس انتخاب‌های فعلی و وضعیت کنونی حساب
        </p>
      </div>

      <div className="card">
        <h3 className="chart-title">دارایی‌ها</h3>
        <p className="net-available-section-hint">
          مواردی که در مجموع دارایی‌ها لحاظ شوند
        </p>
        <ToggleChipGroup
      ...
      <div className="card">
        ...
        <p className="net-available-section-hint">
          مواردی که از دارایی قابل اتکا کسر شوند
        </p>
        <ToggleChipGroup
      ...
      {loading && !financial && (
        <p className="empty-text">در حال بارگذاری...</p>
      )}
    </div>
  );
}
